...
def delete_client_cert(name):
    ...
    if name not in locations: return
    ...
    try:
        revoke_resp = subprocess.run([ 
            "openssl", "ca", "-revoke", str(crt_path),
        ...
        try:
            ...
            log(CERTS_LOGPATH, f"Erro inesperado ao remover o diretório {client_dir}: {str(e)}")
...
def add_client_cert(name):
    ...
    try:
        ...
        csr_resp = subprocess.run([
            "openssl", "req", "-new", "-sha256", "-key", str(key_path),
            "-out", str(csr_path), "-subj", subj
        ], check=True, capture_output=True, text=True)
        log(CERTS_LOGPATH, f"CSR gerado para {name}: {csr_resp.stdout if csr_resp.stdout else 'Sucesso'}")

        cert_resp = subprocess.run([
            "openssl", "x509", "-req", "-sha256", "-days", "365",
            "-in", str(csr_path), "-CA", str(CA_PATH / "ca.crt"),
            "-CAkey", str(CA_PATH / "ca.key"), "-CAcreateserial",
            "-out", str(crt_path)
        ], check=True, capture_output=True, text=True)
        log(CERTS_LOGPATH, f"Certificado assinado para {name}: {cert_resp.stdout if cert_resp.stdout else 'Sucesso'}")
...
def get_client_p12_path(name):
    ...
    if name not in locations: abort(404, description="Cliente não encontrado")
    client_dir = Path(CLIENTS_PATH) / name
